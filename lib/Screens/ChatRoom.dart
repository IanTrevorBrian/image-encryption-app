...
class ChatRoom extends StatelessWidget {
  ...
  Future uploadImage() async {
    ...
    // Encrypt the image bytes with AES
    final encrypter = encrypt.Encrypter(encrypt.AES(_key));
    final encrypted = encrypter.encryptBytes(imageBytes, iv: _iv);

    // Save the encrypted image URL to Firebase Firestore
    await _firestore
        .collection('chatroom')
        .doc(chatRoomId)
        .collection('chats')
  ...
  void onSendMessage() async {
    if (_message.text.isNotEmpty) {
      final encrypter = encrypt.Encrypter(encrypt.AES(_key));
      final encryptedMessage = encrypter.encrypt(_message.text, iv: _iv);
      Map<String, dynamic> messages = {
        "sendby": _auth.currentUser!.displayName,
        "message": base64Encode(encryptedMessage.bytes),
        "iv": base64Encode(_iv.bytes),
        "type": "text",
        "time": FieldValue.serverTimestamp(),
      };
